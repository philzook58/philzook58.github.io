...
provenance
symbolic intervals
egglog as a proof assistant
induction principles

proof egraph - a non destructive egraph not used for matching. Keeps non canonical things around.

A notion of "proof egraph" was mentioned yesterday. This is not familiar to me but sounded very interesting. The idea was that you can keep around uncanonized enodes for the purposes of proof recording? Is this related to a previously discusses notion in the context of contextual equality where 

View
Equality on the nose vs setoid equality. Can we see 


let var(e) = e in
var(e)

2x + 2x ==> let v(2x) = 2x in v(2x) + v(2x)
lam body  is both variable decl and body decl

var(body) is the variable bound here.
Can tie the know using an iniital name, but then everthing works
\x. x 
var x = x + lam x
\x y. x + y
lam (lam (x + y), var (x + y) = y, var (lam x + y) = x
We have a graph where varaibles point to binder site, which is what I want.
app(lam(B),Z) -> well, it doesn't help us with substitution.
subst(var(B), Z).
We sort of have the context in the term. the relevant context is captured inside the varaibles themselves.
sum(x -> sum (y -> x + y)). Hmm. Messing with the binder is still really bad.

So, what do we have: 1. no fresh counter 2. not so much integer juggling.
var has name unlike de bruijn.

It's capturing the "opposite" of what I was suggesting for 
Any rewriting you do inside the term, 


convert the lambda into a let + a variable rename op you push down?

This lambda rep isn't finding alpha equivalent terms.
alpha equiv terms are graph isomrphic. Doesn't help much
...
```python
def add(x,y):
  return z, "FROM add(x,y,z) WHERE x=y & y=z"
  

x, y = Vars("x y")


add = Function("add", 3) #call create table.
rules = [
  add(y,x) <= add(x,y),
...
create table add(x integer, y integer , z integer);
...
univalence as a 
x = y :- f(g(x)) = x, g(f(y)) = y.
No the other direction is the important one?
f(g(x)) = x, :- x = y. 

x = y doesn't make sense as a thing to do. ( because we're proof irrelevant? ) 
External rebuild is a problem.
(x = y)@P

Proof. What is proof of an equality. We could leave holes, especially if they are easily reconstructible by a lesser process.
