...
#Rayleigh-Ritz Variational Solver - Philip Zucker 2011
...
class minimizeWrapper:
    ...
    def norm(self, xfine, x, y, weight = (lambda x:1)): #Gives back the norm
...
plt.figure(1)
actual = np.sin(xfine)/np.sqrt(mini.norm(xfine,xfine,np.sin(xfine)))

#Attempts Solution for different # of parameters
for i in range(2,7):
    size = i
