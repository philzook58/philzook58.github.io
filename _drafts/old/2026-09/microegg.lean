...
-- hmm. I was hoping this would solve accidentail eg capture, but it doesn't
def makeset {a} (egref : ST.Ref a EGraph) : ST a EId :=
  do
  let eg <- egref.get
  let id := eg.uf.size
  egref.set {eg with uf := eg.uf.push id}
  return id
