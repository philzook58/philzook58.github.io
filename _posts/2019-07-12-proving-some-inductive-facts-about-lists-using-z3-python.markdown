...
SMT works by now having the boolean variables of the SAT solver contain inner structure, like the boolean p actually represents the fact $latex x + y < 5$. During the search process it can take the pile of booleans that have been set to true and ask a solver (maybe a linear programming solver in this case) whether those facts can all be made true in the underlying theory. This is an extra spice on top of the SAT search.
...
And here is some code where I believe I'm showing that some simple list functions like reverse, append, and length have some simple properties like $latex \forall t. rev (rev(t)) == t $.
