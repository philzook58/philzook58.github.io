...
There is a conventional story for explaining SMT where you explain SAT solvers first and then how to extend them with theories. I think it's kind of interesting to instead take the starting point as the theory combination problem and totally ignore the SAT part. Half or more of an introductory talk is about how backtracking and CDCL kind of works. Our solvers in this post will only consider problems of the form `p /\ q /\ not r /\ ...`. This seems more relevant for the needs of a saturating solver like EMT anyhow.
...
If you make multiple `Solver` objects in z3, they do not communicate to each other. However, we are staying entirely inside of z3, so some things remain easier to translate between solvers than if the solvers, terms, and literals were truly foreign to each other.

The basic steps of [Nelson Oppen](https://web.stanford.edu/class/cs357/lecture11.pdf) are purification and then propagation. Purification separates statements into theory specific parts by flattening them using fresh symbols, and then dispatching them to the appropriate solver based on some criteria.

This is a routine to flatten an expression
...
In the following, I have two solver objects inside the combination solver. When you `add` a constraint, it flattens it and then dispatch based on the function symbol to the solver that should be responsible for that symbol.
