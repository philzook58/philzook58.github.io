...
I think I've kind of been hitting a watershed of understanding of
...
Here <https://www.philipzucker.com/lean_smt/> "Using Lean like an external smt solver", I just dumped out a lean file. But what am I missing to use lean to write symbolic executors and stuff in a more intimate less hacky way?

In short, the solution is that I want to always be working in `TermElabM` or `CommandElabM` which are "more powerful" than `IO`
`main` is a trap. The work I want to do is in the elaborator, when you have the full power to query and manipulate `Syntax`, `Expr`, unification and tactics. A lightweight way to achieve my external Lean as tool is to have a top level `run_cmd` / `run_elab` rather than `main` and use `lake env lean mycmd.lean`. I don't think this is Lean not making things possible so much as it being a pretty high barrier beast and not knowing how to map it onto things I already know
