...
I could perhaps work on using the annotations available in extended asm to read from C variables, but I'd need to translate from the GAS names of registers to the Pcode names.  Not insurmountable if useful. Here I'm using them (`: "r"(res)`) just to try and force the ordering of where my annotations will be placed.
