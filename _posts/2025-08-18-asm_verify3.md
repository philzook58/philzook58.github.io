...
I wired it up to be a CLI. THere is still the intention to use it as a python library and discharged verification conditions via manual knuckledragger proof if they get hairy or refer to mathematical or higher level side functions (sin, fib, List ADT append, etc).

Here I took a toy swap function example from [frama-C](https://frama-c.com/) WP tutorial section 1.2 <https://www.frama-c.com/download/wp-manual-18.0-Argon.pdf> .
...
I've been sticking to O1, it's the easiest to read. O0 is so verbose.
...
We want to save the initial state in `olda` and `oldb`
...
Here I have not included my no alias constraint. This thing fails with a countermodel. The countermodel prints only the memory locations and variables mentioned in the entry precondition and post condition. It probably would be nice to print more of the countermodel state during the execution. This is doable.
...
The "/tmp/knuckle" include file now pushes the spec information to be saved in a new section. I think this capability is really useful for assembly verification tooling to have a target. It's not that complicated. I've also enabled using local labels if you don't want to pollute your symbol space.
...
You can add inline assembly in C files. At ABI boundaries, you've got a good chance of knowing where things are going. This is again an advantage I recieve from using regular GAS macros on regular assembly files as my specs. I could perhaps work on using the annotations available in extended asm to read from C variables, but I'd need to translate from the GAS names of registers to the Pcode names. Not insurmountable if useful.
