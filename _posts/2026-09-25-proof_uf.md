...
The union find is maintaining a forest. A confusing thing is that this forest _does not_ reflect a spanning tree in the usual implementation of union finds. This is because `union(a,b)` first calls `c = find(a)` and `d = find(b)` and then attaches `c-d` in the union find. But the edge in the _graph_ was `a-b`, not `c-d`.
