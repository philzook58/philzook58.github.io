...
date: 2024-01-01
layout: post
title: "Knuckledragger: Experimenting in A Python Proof Assistant"
---

Something that I'm tinkering with is making a proof assistant in python.

It's very preliminary, but I've place some ideas [here](https://github.com/philzook58/knuckledragger). I have already done too much without writing about it
...
It is also a huge burden for a new user to learn the ideas of formal mathematical proof, _and_ a new language.
...
Something I've experienced in using automated logic solvers (z3, eprover, egglog) is that one can kind of perform nice big steps in a proof like rearranging expressions or facts about linear arithmetic, but that there are not facilities for chaining these results together. You also can't really express axiom schema (like induction!) or definitional mechanisms. You sort of bring the edges close together in file and wave your hands a bit. This is a corner of the design space that I think is interesting. Putting all the meat in the solvers (and trusting that z3, eprover, etc are basically correct, de bruijn be damned) and have a light layer of trustable chaining and metaprogramming on top. This is not an unknown idea. It has been around since the very first solvers in the 50s and is more explored in the programming context of F*, Liquid Haskell, and Dafny.

A traditional inference chaining design can be seen in LCF style kernels, where inference rules are implemented as functions taking in an abstract datatype `proof` and outputting a new piece of `proof`. You can look inside `proof` to see the `formula`  `formula_of_proof : proof -> formula` but there is not explicit function to do the opposite (except maybe an unsafe `axiom` primitive). In a language like Ocaml, the immutability and type system guarantee  . See Jon Harrison's Handbook for more.
