...
The documentation, schematics, pictures for the computer is here <https://github.com/wuxx/TD4-4BIT-CPU>. It's a little sparse though. I can imagine a student getting overwhelmed. So I thought it would be helpful to write some longer form notes here.
...
The ROM is the bank of 16 dip switches. This is the entirety of the program available to you, which makes it tricky to do much. Pins 5-8 of each dip switch are an opcode, selecting between `ADD`, `MOV` `IN` `OUT` `JNC` `JMP` instructions. These bits go through some combinatorial circuit to control 1. What signal enters the addition unit 2. which register latches in the output of the addition unit. Bits 1-4 of the dip switch are the immediate value for the instruction, which is always piped into the adder.
