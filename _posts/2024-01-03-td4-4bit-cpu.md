...
I bought a cute little 4 bit cpu kit from aliexpress called the TD4 <https://www.aliexpress.us/item/3256804578823017.html?gatewayAdapt=glo2usa4itemAdapt> . There's a couple versions available.
...
We took two sessions just soldering it up. The image ![](https://github.com/wuxx/TD4-4BIT-CPU/blob/master/doc/TD4-black-1.jpg) plus the schematics didn't make this conceptually too hard. ![](https://github.com/wuxx/TD4-4BIT-CPU/blob/master/doc/cpu_td4.jpg). The thing that gave us the most pause was figuring out the directionality of the surface mount diodes. The thing that gave us the most trouble was soldering on the USB connector. I'd advise doing this before putting on the IC sockets, because  it got in the way of the iron. The middle pins of the USB are unconnected, so you can just blast them if need be. Which chip goes where can be seen by inspecting the schematic for part number and IC number. The notch aligns with the notch printed on the PCB board. It basically worked the first time, minus an intermittent power connection on the usb.

At a a high level here's how it works
![](https://github.com/wuxx/TD4-4BIT-CPU/blob/master/doc/block%20diagram.png)
