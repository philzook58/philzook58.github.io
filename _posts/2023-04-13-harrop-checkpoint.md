...
1. It needs the extrude check to prevent the gensym `X` from escaping out of `B`.
2. Copies of the pi share the same `X` 
3. Accidental variable collision `pi(X,pi(X,true))` will not do the right thing. 

Another crucial feature of the proper treatment of these forall fresh variables is the extrude check. In it's usage for existentials, a logic variable is supposed to be a stand-in for some lazily filled in concrete ground term. But this ground term should only contain symbols that existed at the time you traversed the `ex` binder, and hence constants generated by inner `pi` are not acceptable candidates. This check is similar in some ways to the infamous [occurs check](https://www.swi-prolog.org/pldoc/doc_for?object=unify_with_occurs_check/2) in that it is a limitation on unification, really required for logical soundness, and kind of annoying.

One way to treat this is to do it lazily, with an extrude check that checks the term that was inferred for a crossed
...
This is definitely going to be possible, you can build an intepreter for any language in prolog with enough muscle grease, but the ultimate goal is to remove the interpreter.

We use a couple tricks. Instead of `pi(X,B)`, we use a HOAS-like `pi(L)`. `L` is a goal that cann be `call`ed with 2 arguments. The first is the instantiation of `X`, and the second is an output parameter `G` that will be unified with `B` with `X` replaced. We can't really have `L` "return" `G = B[t/X]` because returning isn't a thing in prolog, so this is what we have to do.
...
For the extrude check, since we explicitly introduce the existential variables with `ex(L)`, we can check for scope extrusion after calling `L` is done.

We have two extra context parameters we thread in the interpeter. One is the signature which holds gensymed variables, and the other is the program context which holds predicates introduced by .
