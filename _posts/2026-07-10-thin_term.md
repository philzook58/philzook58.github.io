...
- Cyclic terms <https://www.philipzucker.com/arena_coegraph2_rational/> . Is arena vs hash cons vs egraph vs pointery structs really a difference in "term"? Probably not. This feels like a renegotiation of what a term is on a different axis than most of these other things in that it isn't changing the node structure really, but changes how the whole thing is store or what the definition of equality of terms is in a more global way.
...
An alternaitve design would be to mutually define

```
type lift = {thin : bool list; t : term} 
and term = {f : string; args : lift list }
```

Maybe that would have made some of my manipulations nicer and map more cleanly onto lifts as actual terms that just show up everywhere.

I may have defined matching through nontirival thinnings wrong. Maybe I want to allow pattern variables to be something that exists inside the ocntext in which I could have  `f(?F[x,y], ?F[y,z])` work. Interesting that this seems like a restricution of miller patterns to not allow permutations of variable usage in nonlinear patterns?
