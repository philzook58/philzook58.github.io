...
Ok, so the above is just a core. A proof assistant it hardly make. There is a butt ton of infrastructure needed on top of any kernel. Starting with an automated theorem prover does get you moving and is great glue for other subsystems.
...
all `assert`s should be `raise`s I suppose.
