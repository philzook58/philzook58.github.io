...
def proc_stmt(stmt : IRStmt):
    ...
    if isinstance(stmt, NoOp):
        ...
        #return f"if({proc_expr(stmt.guard)}){{ goto {label(stmt.dst.value)}; }}"
        return f"if({proc_expr(stmt.guard)}){{ state->gr[{stmt.offsIP}] = 0x{stmt.dst.value:x}; break; /*goto {label(stmt.dst.value)}; */ }}" 
